...
void make_model(Model* model_ptr){
    ...
    model_ptr->create_cartesian_grid(5, 5, 1, 5, 5, false);//last two fives determine size of the grid //Bookmark
    ...
    long long int n_recept_array [25] = {};
	long long int n_b_array[25] ={};
    //some pointer n_cells
    for(int ii = 0; ii < model_ptr->n_cells; ii++){
        double receptorpower = ((static_cast<double>(ii*n_powers))/24) + low_power;
    ...
    model_ptr->run(50000000000, 100000, false, folder_name);
