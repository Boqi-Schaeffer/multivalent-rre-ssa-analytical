...
void make_model(Model* model_ptr){
    ...
    model_ptr->load_model("3 arms konsol 5.00e-08 konsur 1.00e-04_model.txt"); //! This is where the model.txt file is specified
    ...
    long long int n_recept_array [6] = {};
	long long int n_b_array[6] ={};
    //some pointer n_cells
    for(int ii = 0; ii < model_ptr->n_cells; ii++){
        double receptorpower = ((static_cast<double>(ii*n_powers))/5) + low_power;
    ...
    model_ptr->run(50000000000, 1, false, folder_name);
